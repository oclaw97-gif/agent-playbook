...
# For each repo, fetches merged PR review comments since last collection.
...
while IFS=$'\t' read -r repo since; do
  ...
  # Fetch merged PRs updated since last collection
  pr_numbers=$(gh api "repos/$repo/pulls?state=closed&sort=updated&direction=desc&per_page=100" \
